...
class GeneticAlgorithm:
    ...
    def __init__(self, popsize, city_list, planned_epochs = 100):
        ...
        self.os_tries = 1000 # maximum generations for search for successful offsprings - per epoch
    ...
    def breed_os(self, n, *, mutation_rate, smoothen_chances = 0) -> List[RouteUnit]:
        """
        ...
        successful = []
        successful_size = int(n * self.success_ratio)
        rest = deque(maxlen= n - successful_size)

        n_tries = 0

        while len(successful) < successful_size:
            n_tries += 1
            if n_tries > self.os_tries:
                raise StopIteration
    ...
    def step(self, eliteSize, mutationRate):
        ...
        next_generation += self.breed_os(new_size - eliteSize, mutation_rate=mutationRate)
